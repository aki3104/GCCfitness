  ...
  <v-container>
    <v-row align="center" justify="center">
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              ...
              <v-flex xs12>
                <v-text-field
                  :placeholder="tosigninUser.email"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="info" @click="updateFirebase(user)" dark>更新</v-btn>
        </v-card-actions>
      </v-card>
    </v-row>
  </v-container>
...
export default {
  ...
  methods: {
    ...
    ...mapActions("user", ["updateFirebase"]),
    updateParams(event, keyName) {
      this.update({ value: event, keyName });
    },
  },
  computed: {
    ...mapGetters("user", ["users", "signinUser"]),
    user() {
      return this.users[0];
    },
    tosigninUser() {
